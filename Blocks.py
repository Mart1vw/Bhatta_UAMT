...
def upSampleConv_Res(
    nin, nout, upscale=2, bias=False, BN=False, ws=False, activ=nn.LeakyReLU(0.2)
):
...
def conv_block_Asym_Inception(in_dim, out_dim, kernel_size, padding, dilation=1):
    model = nn.Sequential(
        nn.Conv2d(
            ...
            padding=tuple([padding * dilation, 0]),
            dilation=(dilation, 1),
        ),
        nn.BatchNorm2d(out_dim),
        nn.ReLU(),
        nn.Conv2d(
            ...
            padding=tuple([0, padding * dilation]),
            dilation=(dilation, 1),
        ),
        nn.BatchNorm2d(out_dim),
        nn.ReLU(),
...
def conv_block_Asym_Inception_WithIncreasedFeatMaps(
